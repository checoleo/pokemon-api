import * as React from "react"

const SaveSvg = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={288} height={288} {...props}>
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={288}
      height={288}
      viewBox="0 0 486 486"
      {...props}
    >
      <path
        fill="#fff"
        d="M473.7 485.75c6.8 0 12.3-5.5 12.3-12.3v-359.8c0-3.6-1.6-7-4.3-9.3L363 2.85c-.2-.2-.4-.3-.6-.4-.3-.2-.5-.4-.8-.6-.4-.2-.7-.4-1.1-.6-.3-.1-.6-.3-.9-.4-.4-.2-.9-.3-1.3-.4-.3-.1-.6-.2-.9-.2-.8-.1-1.5-.2-2.3-.2H12.3C5.5.05 0 5.55 0 12.35v461.3c0 6.8 5.5 12.3 12.3 12.3h461.4v-.2zm-89.2-24.5h-283v-184.1c0-3.7 3-6.6 6.6-6.6h269.8c3.7 0 6.6 3 6.6 6.6v184.1zM161.8 24.45h180.9v127.8c0 .8-.6 1.4-1.4 1.4h-178c-.8 0-1.4-.7-1.4-1.4V24.45h-.1zm-137.2 0h112.8v127.8c0 14.3 11.6 25.9 25.9 25.9h178c14.3 0 25.9-11.6 25.9-25.9V38.75l94.2 80.6v341.9H409v-184.1c0-17.2-14-31.1-31.1-31.1H108.1c-17.2 0-31.1 14-31.1 31.1v184.2H24.6V24.45z"
        className="color000 svgShape"
      />
      <path
        fill="#fff"
        d="M227.4 77.65h53.8v32.6c0 6.8 5.5 12.3 12.3 12.3s12.3-5.5 12.3-12.3v-44.8c0-6.8-5.5-12.3-12.3-12.3h-66.1c-6.8 0-12.3 5.5-12.3 12.3s5.6 12.2 12.3 12.2zm77.1 245.2h-123c-6.8 0-12.3 5.5-12.3 12.3s5.5 12.3 12.3 12.3h123c6.8 0 12.3-5.5 12.3-12.3s-5.5-12.3-12.3-12.3zm0 64.9h-123c-6.8 0-12.3 5.5-12.3 12.3s5.5 12.3 12.3 12.3h123c6.8 0 12.3-5.5 12.3-12.3s-5.5-12.3-12.3-12.3z"
        className="color000 svgShape"
      />
    </svg>
  </svg>
)

export default SaveSvg;
